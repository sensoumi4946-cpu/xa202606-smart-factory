...
def generate_message(device_id: str, subsystem: Subsystem) -> UnifiedMessage:
    if subsystem == Subsystem.TEMP_HUMIDITY:
        measurements = [
            Measurement(type=MeasurementType.TEMPERATURE, value=round(random.uniform(15.0, 40.0), 1), unit=Unit.CELSIUS),
            Measurement(type=MeasurementType.HUMIDITY, value=round(random.uniform(20.0, 90.0), 1), unit=Unit.PERCENT),
        ]
        ...
        measurements = [
            ...
            Measurement(type=MeasurementType.LIGHT_STATE, value=light, unit=Unit.BOOLEAN),
        ]
        ...
        measurements = [
            ...
            Measurement(type=MeasurementType.COMBUSTIBLE_GAS, value=cgas, unit=Unit.PPM),
        ]
...
def publish_single(client: mqtt.Client, subsystem: Subsystem, device_id: str):
    ...
    for m in msg.measurements:
        ...
        payload = json.dumps({"type": m.type.value, "value": m.value, "unit": m.unit.value})
...
def main():
    ...
    parser.add_argument("--interval", type=float, default=DEFAULT_INTERVAL, help="Interval between batches (seconds)")
