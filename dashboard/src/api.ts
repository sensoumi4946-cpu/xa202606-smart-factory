...
export async function apiFetch(url: string, init?: RequestInit): Promise<Response> {
    ...
    for (const [key, entry] of _cache) {
        if (Date.now() - entry.at >= COALESCE_MS)
            _cache.delete(key);
    }
...
export async function fetchLatest(deviceId?: string): Promise<LatestDevice[]> {
...
export async function fetchLatestDeduped(deviceId?: string): Promise<LatestDevice[]> {
    if (deviceId)
        return fetchLatest(deviceId);
    if (!_pendingLatest) {
...
export async function rawRequest(method: string, path: string, body?: unknown): Promise<RawResult> {
...
export async function probeFuseki(): Promise<boolean> {
    try {
        const resp = await apiFetch(`${BASE_URL}api/v1/semantic?view=sensor-observations`);
        return resp.ok;
    }
    catch {
        return false;
...
export async function runSparqlQuery(query: string): Promise<SparqlResult> {
    ...
    const bindings: Array<Record<string, {
        value: string;
